...
# Remove duplicate songs from full table
  ...
  '''
    ...
    FROM full_acoustic_brainz
    JOIN spoticry.listening_songs 
    ON spoticry.listening_songs.cleaned_title = full_acoustic_brainz.cleaned_title AND
        spoticry.listening_songs.cleaned_artist = full_acoustic_brainz.cleaned_artist
    JOIN spoticry.Sentiments 
    ON spoticry.listening_songs.song_id = spoticry.Sentiments.song_id;
  '''
    c.execute(merge)
